...
import {clearPizzas, minusCartItem, plusCartItem, removePizzas} from '../../../redux/cart-reducer'
//import {removePizzas} from '../../../redux/cart-reducer'
import emtyCart from '../../../assets/img/empty-cart.png'
...
const Cart = () => {
  ...
  const addedPizzas = keys.map(key => items[key].items[0])
...
<path d="M11.6666 9.16667V14.1667" stroke="#B6B6B6" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
                ...
                <span onClick={onClearPizzas}>Очистить корзину</span>
              </div>
            </div>
            <div className="content__items">
            {addedPizzas.map(obj => <CartItem id={obj.id} name={obj.name} type={obj.type} size={obj.size} ItemsCount={items[obj.id].items.length} totalPrice={items[obj.id].totalPrice} onRemovePizza={onRemovePizza} onMinus={onMinusCartItem} onPlus={onPlusCartItem}/>)}
              </div>
...
<path d="M7 13L1 6.93015L6.86175 1" stroke="#D3D3D3" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
          ...
          : <div className="cart cart--empty">
            ...
            <img src={emtyCart} alt="Empty cart" />
